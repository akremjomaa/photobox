...
export async function next() {
  if (currentElement.nextElementSibling === null) {
      if (dataForLightbox === undefined){
          var dataInit = dataGlobal
      } else {
          var dataInit = dataForLightbox
      }

      let data = await nextGalery(dataInit);
...
export async function prev() {
  if (currentElement.previousElementSibling === null) {
      ...
      let data = await prevGalery(dataInit);
      display_galerie(data.photos);
